...
public struct TronSigner: CustomStringConvertible {
    ...
    public init(privateKey: Data) throws {
        guard TronSigner.isValidPrivateKey(privateKey), let publicKey =
                SECP256K1.privateToPublic(privateKey: privateKey, compressed: false) else {
...
extension TronSigner {
    public func signDigest(_ hash: Data) -> Data? {
        let signedData = SECP256K1.signForRecovery(hash: hash, privateKey: privateKey, useExtraEntropy: false)
